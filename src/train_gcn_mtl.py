...
LEARNING_RATE = 0.001
MODE = 'train'
STORAGE_PATH = '../models/gcn_mtl/'
LOG_FILE = 'log_gcn_mtl.txt'
EPOCH_FILE = 'epoch_gcn_mtl.txt'
...
#                print(file)
    ...
    if MODE == 'train':
        ...
        model = MTLGCN(node_features=NUM_NODE_FEATURES, leadtime_range=LEADTIME_RANGE).to(device)
        ...
        for epoch in range(EPOCHS):
            ...
            torch.save(model.state_dict(), STORAGE_PATH + 'gcn_mtl_{}.pth'.format(epoch))
        ...
        model.load_state_dict(torch.load(STORAGE_PATH + 'gcn_mtl_{}.pth'.format(load_epoch)))
