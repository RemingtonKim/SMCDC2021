...
from models import LeadRecurrentGCN
...
LEARNING_RATE = 0.001
MODE = 'train'
STORAGE_PATH = '../models/rec_lead/'
LOG_FILE = 'log_rec_lead.txt'
EPOCH_FILE = 'epoch_rec_lead.txt'
...
if __name__ == '__main__':
    ...
    if MODE == 'train':
        ...
        model = LeadRecurrentGCN(node_features=NUM_NODE_FEATURES, leadtime_range=LEADTIME_RANGE).to(device)
        ...
        for epoch in range(EPOCHS):
            ...
            torch.save(model.state_dict(), STORAGE_PATH + 'rec_lead_{}.pth'.format(epoch))
            ...
            log_and_print('Validation loss : {}'.format(sum(val_cost)/len(val_cost)), log_file)
        ...
        model.load_state_dict(torch.load(STORAGE_PATH + 'rec_lead_{}.pth'.format(load_epoch)))
        model.eval()
