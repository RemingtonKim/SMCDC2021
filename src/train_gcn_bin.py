...
LEARNING_RATE = 0.001
MODE = 'train'
STORAGE_PATH = '../models/gcn_bin/'
LOG_FILE = 'log_gcn_bin.txt'
EPOCH_FILE = 'epoch_gcn_bin.txt'
...
#                print(file)
    ...
    if MODE == 'train':
        ...
        model = BinGCN(node_features=NUM_NODE_FEATURES).to(device)
        ...
        for epoch in range(EPOCHS):
            ...
            torch.save(model.state_dict(), STORAGE_PATH + 'gcn_bin_{}.pth'.format(epoch))
        ...
        model.load_state_dict(torch.load(STORAGE_PATH + 'gcn_bin_{}.pth'.format(load_epoch)))
        model.eval()
