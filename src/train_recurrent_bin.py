...
from models import BinRecurrentGCN
...
LEARNING_RATE = 0.001
MODE = 'train'
STORAGE_PATH = '../models/rec_bin/'
LOG_FILE = 'log_rec_bin.txt'
EPOCH_FILE = 'epoch_rec_bin.txt'
...
#                print(file)
    ...
    if MODE == 'train':
        ...
        model = BinRecurrentGCN(node_features=NUM_NODE_FEATURES).to(device)
        ...
        for epoch in range(EPOCHS):
            ...
            for index in tqdm(range(0, len(train), BATCH_SIZE)):
                ...
                bin_yhat = model(x, batch, src, dst)
                
                bin_yhats += bin_yhat.detach().cpu().tolist()

                loss = F.binary_cross_entropy(bin_yhat, bin_y)
            ...
            log_and_print('Training Bin ROC AUC : {}'.format(metrics.roc_auc_score(bin_ys, bin_yhats)), log_file)

            torch.save(model.state_dict(), STORAGE_PATH + 'rec_bin_{}.pth'.format(epoch))
            ...
            log_and_print('Validation Bin ROC AUC : {}'.format(val_auc), log_file)
        ...
        model.load_state_dict(torch.load(STORAGE_PATH + 'rec_bin_{}.pth'.format(load_epoch)))
        model.eval()
        ...
        accuracy = metrics.accuracy_score(bin_ys, bin_yhats_hard)
